...
void CHud__DrawScriptText_hook(uintptr_t thiz, uint8_t unk)
{
	...
	if (pGame && pNetGame)
	{
		...
		if (g_pWidgetManager)
		{
			if (!g_pWidgetManager->GetSlotState(0))
			{
				g_pWidgetManager->New(WIDGET_ENTER_PASSENGER, 
					...
					pSettings->GetReadOnly().fButtonEnterPassengerSize, "hud_dance");
			}
			...
			if (!g_pWidgetManager->GetSlotState(WIDGET_MICROPHONE))
			{
				if (g_IsVoiceServer())
				{
					...
					g_pWidgetManager->GetWidget(WIDGET_MICROPHONE)->SetTexture("gui", "voice_button");
				}
			}
...
int CWidgetButtonEnterCar_Draw_hook(uintptr_t thiz)
{
	if (g_pWidgetManager)
	{
		CWidget* pWidget = g_pWidgetManager->GetWidget(WIDGET_ENTER_PASSENGER);
		if (pWidget)
		{
			if (*(uint8_t*)(thiz + 76))
			{
...
void (*DoHeadLightReflectionSingle)(VEHICLE_TYPE* thiz, MATRIX4X4* matrix, unsigned char lightId);
void DoHeadLightReflectionSingle_hook(VEHICLE_TYPE* vehGta, MATRIX4X4* matrix, unsigned char lightId)
{
	bool m_bOldColor = false;
	uint8_t m_aOld[3];
	CVehicle* pVehh = nullptr;
	if (pNetGame)
	{
		if (pNetGame->GetVehiclePool())
		{
			uint16_t vehid = pNetGame->GetVehiclePool()->FindIDFromGtaPtr(vehGta);
			CVehicle* pVeh = pNetGame->GetVehiclePool()->GetAt(vehid);
			if (pVeh)
			{
				pVehh = pVeh;
				if (pVeh->m_bHeadlightsColor)
				{
					m_aOld[0] = pVeh->m_bHeadlightsR;
					m_aOld[1] = pVeh->m_bHeadlightsG;
					m_aOld[2] = pVeh->m_bHeadlightsB;
					m_bOldColor = true;
				}

				if (pVeh->m_bStrobsPolice)
				{
					if (lightId == 0)
					{
						pVeh->SetHeadlightsColor(255, 0, 0);
					}
					else
					{
						pVeh->SetHeadlightsColor(0, 0, 255);
					}
				}
			}
		}
	...
	DoHeadLightReflectionSingle(vehGta, matrix, lightId);

	if (m_bOldColor)
	{
		if (pVehh)
		{
			pVehh->SetHeadlightsColor(m_aOld[0], m_aOld[1], m_aOld[2]);
		}
	}
}
...
void InstallHooks()
{
	...
	SetUpHook(g_libGTASA + 0x00519320, (uintptr_t)DoHeadLightReflectionSingle_hook, (uintptr_t*)& DoHeadLightReflectionSingle);
