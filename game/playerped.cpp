...
void RwMatrixScale(MATRIX4X4* matrix, VECTOR* scale)
{
	...
	result->right.Y = result->right.Y * scale->X;
	result->right.Z = result->right.Z * scale->X;
	result->up.X = scale->Y * result->up.X;
	result->up.Y = scale->Y * result->up.Y;
	result->up.Z = scale->Z * result->up.Z;
	result->at.X = scale->Z * result->at.X;
	result->at.Y = scale->Z * result->at.Y;
	matrix->at.Z = scale->Z * matrix->at.Z;
	result->flags &= 0xFFFDFFFC;
}
