...
CVehicle::~CVehicle()
{
	...
	if (m_pVehicle)
	{
		...
		if (m_pTrailer)
		{
			// detach trailer (äîïèëèòü)
		}

		// òóò åùå êàêàÿ-òî èíòåðåñíàÿ øíÿãà
...
void CVehicle::ApplyVinyls(uint8_t bSlot1, uint8_t bSlot2)
{
	...
	if (bSlot1 != 255)
	{
		...
		ApplyTexture("ret_v_body1", &szTex[0]);
	}

	if (bSlot2 != 255)
	{
		sprintf(&szTex[0], "v_cust_body_%d", bSlot2);
		ApplyTexture("ret_v_body2", &szTex[0]);
	}
...
void CVehicle::ApplyToner(uint8_t bSlot, uint8_t bID)
{
	...
	if (bID == 0)
	{
		sprintf(&szOld[0], "ret_t_%d", bSlot);
...
{
	((void(*)(uintptr_t, int, int))(g_libGTASA + 0x004F9380 + 1))((uintptr_t)((uintptr_t)m_pVehicle + 1456), iLight, iState); // CAutomobile::OpenDoor
}
